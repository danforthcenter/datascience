...
Local, on-premise computing on the Data Science infrastructure is through the HTCondor cluster.
...
machines and job execution machines. Users do not interact with this server (`aquaria`) directly.
* **Scheduler server**. This server manages a job queue and exchanges job information with the central manager. 
Multiple servers can run the job scheduling service. Schedulers run on `stargate` and `jupyter`.
* **Execution server**. These are the servers where jobs are run. Jobs submitted from a scheduler (`stargate` or `jupyter`)
are  analyzed by the central manager (`aquaria`) to determine which execution servers can support the job 
...
In the example above, `myenv-program` is only installed within the `myenv` conda environment, so if 
`myenv-program` is submitted as a program in a job to the HTCondor cluster, the job will fail because the
`myenv` conda environment cannot be activated automatically first. To get around this issue, you can create a
shell script that activates the environment and then runs the application.

Following the example above, a shell script (`myenv-program.sh`) that runs `myenv-program` would look like this if you are
using [Miniforge](https://conda-forge.org/download/):
...
getenv           = true
executable       = /bin/bash
arguments        = myenv-program.sh --options
log              = myenv-program.log
output           = myenv-program.out
error            = myenv-program.error
request_cpus     = 1
request_memory   = 1G
